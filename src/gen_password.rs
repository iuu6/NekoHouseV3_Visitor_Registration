...
impl std::fmt::Display for PasswordType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PasswordType::Temp => write!(f, "临时密码"),
            PasswordType::Times(times) => write!(f, "次数密码({}次)", times),
            PasswordType::Limited(duration) => {
                let hours = duration.num_hours();
                let minutes = duration.num_minutes() % 60;
                if minutes == 0 {
                    write!(f, "时效密码({}小时)", hours)
                } else {
                    write!(f, "时效密码({}小时{}分钟)", hours, minutes)
                }
            },
            PasswordType::Period(end_time) => write!(f, "期间密码(至{})", end_time.format("%Y-%m-%d %H:%M")),
            PasswordType::LongtimeTemp(end_time) => write!(f, "长期临时密码(至{})", end_time.format("%Y-%m-%d %H:%M")),
        }
    }
}
